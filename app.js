...
const questions = [
 ...
 {type:'general',visual:'lane-red-x',q:'What does a red X above a lane mean?',a:['Use the lane only to pass','The lane is closed to traffic','Exit from this lane','The lane is about to open'],correct:1,why:'Never drive in a lane marked by a red X.'},
 {type:'general',visual:'dark-signal',q:'What should you do at a traffic signal that is completely dark?',a:['Proceed without stopping','Treat it as a yield','Treat it as an all-way stop','Wait until it turns on'],correct:2,why:'Treat an out-of-service signal as an all-way stop unless an officer or temporary control directs otherwise.'},
 ...
 {type:'general',visual:'steady-yellow-signal',q:'A traffic light turns yellow as you approach. What does it mean?',a:['Speed up before red','Stop if you can do so safely','You always have right-of-way','Cross traffic must wait indefinitely'],correct:1,why:'A steady yellow warns the signal is changing to red. Stop if you can do so safely.'},
 {type:'general',visual:'broken-yellow-line',q:'A broken yellow center line means:',a:['Passing is never allowed','Passing is allowed when safe','Traffic moves in the same direction','The lane is for parking'],correct:1,why:'A broken yellow line separates opposing traffic and allows passing when the way is clear and it is otherwise legal.'},
 {type:'general',visual:'double-yellow-lines',q:'Two solid yellow center lines mean:',a:['Passing is allowed both ways','Passing is prohibited in both directions','Traffic moves one way','Only trucks may pass'],correct:1,why:'Double solid yellow lines prohibit passing in both directions, though turns into driveways may be permitted when safe.'},
 ...
 {type:'general',visual:'stop-line',q:'Where should you stop when there is a stop line?',a:['After the crosswalk','At the stop line','In the intersection','Where you can see best, even past the line'],correct:1,why:'Stop before the marked stop line. If none exists, stop before the crosswalk or intersection.'},
...
questions.push(
 ...
 {type:'general',topic:'Signals · Traffic signals',visual:'flashing-yellow-signal',q:'What does a flashing yellow signal mean?',a:['Stop completely','Slow down and proceed with caution','The road is closed','Speed up before it turns red'],correct:1,why:'A flashing yellow identifies a higher-than-normal hazard; slow and proceed cautiously.'},
 {type:'general',topic:'Signals · Lane use signals',visual:'lane-yellow-arrow',q:'A yellow diagonal downward arrow over a lane tells you to:',a:['Enter that lane immediately','Move out of that lane as soon as safely possible','Stop under the signal','Use the lane only for passing'],correct:1,why:'A yellow X or diagonal downward arrow warns that the lane is closing; leave it safely.'},
 ...
 {type:'general',topic:'Markings · Pavement markings',visual:'mixed-yellow-lines',q:'A broken yellow line beside a solid yellow line means:',a:['Passing is allowed from both sides','Passing is allowed only from the side with the broken line','Passing is prohibited from both sides','Traffic flows in the same direction'],correct:1,why:'The broken-line side may pass when safe; the solid-line side may not.'},
 {type:'general',topic:'Markings · Pavement markings',visual:'hov-diamond',q:'A white diamond painted in a lane usually means:',a:['The lane is reserved for special traffic such as HOV vehicles','The pavement is slippery','A railroad crossing is ahead','Parking is allowed'],correct:0,why:'A diamond marks a reserved lane, often for high-occupancy vehicles or another designated use.'},
 {type:'general',topic:'Markings · Painted curbs',visual:'yellow-curb',q:'What does a yellow-painted curb generally mean?',a:['Parking is encouraged','Parking is prohibited or restricted as posted','The curb is privately owned','Only motorcycles may park'],correct:1,why:'Yellow curbs identify areas where parking is prohibited or restricted; obey accompanying signs.'},
...
function signGraphic(question) {
  ...
  if(visual==='lane-red-x') return svg('<rect width="120" height="100" rx="7" fill="#22282d"/><path d="M38 25l44 50M82 25L38 75" stroke="#ef3340" stroke-width="12" stroke-linecap="square"/>');
  if(visual==='lane-yellow-arrow') return svg('<rect width="120" height="100" rx="7" fill="#22282d"/><path d="M34 23l51 52m0 0V51m0 24H61" fill="none" stroke="#ffd329" stroke-width="9" stroke-linejoin="miter"/>');
  if(visual==='dark-signal') return svg('<rect x="38" y="3" width="44" height="94" rx="10" fill="#171b1e" stroke="#f2b134" stroke-width="3"/><circle cx="60" cy="22" r="11" fill="#343a3e" stroke="#080a0b" stroke-width="3"/><circle cx="60" cy="50" r="11" fill="#343a3e" stroke="#080a0b" stroke-width="3"/><circle cx="60" cy="78" r="11" fill="#343a3e" stroke="#080a0b" stroke-width="3"/>');
  if(visual==='steady-yellow-signal' || visual==='flashing-yellow-signal') return svg(`<rect x="38" y="3" width="44" height="94" rx="10" fill="#171b1e" stroke="#f2b134" stroke-width="3"/><circle cx="60" cy="22" r="11" fill="#40272b" stroke="#080a0b" stroke-width="3"/><circle cx="60" cy="50" r="12" fill="#ffd329" stroke="#080a0b" stroke-width="3"/><circle cx="60" cy="78" r="11" fill="#26382e" stroke="#080a0b" stroke-width="3"/>${visual==='flashing-yellow-signal'?'<path d="M25 50h-12m94 0H95M33 23l-9-9m72 72-9-9m9-63-9 9M33 77l-9 9" stroke="#ffd329" stroke-width="4"/>':''}`);
  if(visual==='broken-yellow-line') return svg('<rect width="120" height="100" fill="#555b60"/><path d="M60 3v17m0 14v17m0 14v17m0 14v4" stroke="#ffd329" stroke-width="5"/><path d="M13 0v100M107 0v100" stroke="#fff" stroke-width="3"/>');
  if(visual==='double-yellow-lines') return svg('<rect width="120" height="100" fill="#555b60"/><path d="M55 0v100M65 0v100" stroke="#ffd329" stroke-width="4"/><path d="M13 0v100M107 0v100" stroke="#fff" stroke-width="3"/>');
  if(visual==='mixed-yellow-lines') return svg('<rect width="120" height="100" fill="#555b60"/><path d="M55 0v100" stroke="#ffd329" stroke-width="4"/><path d="M65 3v17m0 14v17m0 14v17m0 14v4" stroke="#ffd329" stroke-width="4"/><path d="M13 0v100M107 0v100" stroke="#fff" stroke-width="3"/>');
  if(visual==='stop-line') return svg('<rect width="120" height="100" fill="#555b60"/><path d="M8 13h78" stroke="#fff" stroke-width="10"/><path d="M18 0v100M91 0v100" stroke="#fff" stroke-width="3"/><path d="M105 12v69" stroke="#777" stroke-width="4"/><polygon points="96,9 108,4 117,9 117,22 108,28 96,22" fill="#c9232a" stroke="#fff" stroke-width="2"/><text x="106.5" y="18" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="5" font-weight="900">STOP</text><path d="M42 76h25" stroke="#dce8ef" stroke-width="3"/><path d="M42 76l8-6v12z" fill="#dce8ef"/>');
  if(visual==='railroad-flashers') return svg('<path d="M60 38v62" stroke="#777" stroke-width="5"/><g transform="translate(60 22) rotate(34)"><rect x="-39" y="-7" width="78" height="14" fill="#fff" stroke="#202326" stroke-width="2"/></g><g transform="translate(60 22) rotate(-34)"><rect x="-39" y="-7" width="78" height="14" fill="#fff" stroke="#202326" stroke-width="2"/></g><rect x="28" y="42" width="64" height="8" rx="4" fill="#202326"/><circle cx="40" cy="46" r="10" fill="#ef3340" stroke="#fff" stroke-width="3"/><circle cx="80" cy="46" r="10" fill="#ef3340" stroke="#fff" stroke-width="3"/><path d="M13 78h94" stroke="#ef3340" stroke-width="7"/><rect x="15" y="73" width="14" height="10" fill="#fff"/><rect x="43" y="73" width="14" height="10" fill="#fff"/><rect x="71" y="73" width="14" height="10" fill="#fff"/>');
  if(visual==='hov-diamond') return svg('<rect width="120" height="100" fill="#555b60"/><path d="M15 0v100M105 0v100" stroke="#fff" stroke-width="3"/><path d="M60 18L82 50 60 82 38 50z" fill="none" stroke="#fff" stroke-width="6"/>');
  if(visual==='yellow-curb') return svg('<rect width="120" height="72" fill="#555b60"/><rect y="72" width="120" height="14" fill="#ffd329"/><rect y="86" width="120" height="14" fill="#b7b9b7"/><path d="M8 58h104" stroke="#fff" stroke-width="3" stroke-dasharray="17 11"/>');
  if(text.includes('eight-sided')) return svg('<polygon points="38,5 82,5 105,28 105,72 82,95 38,95 15,72 15,28" fill="#c9232a" stroke="#fff" stroke-width="5"/><text x="60" y="58" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="21" font-weight="900">STOP</text>');
  if(text.includes('triangular')) return svg('<polygon points="10,12 110,12 60,92" fill="#fff" stroke="#d0202d" stroke-width="7"/><text x="60" y="48" text-anchor="middle" fill="#202326" font-family="Arial,sans-serif" font-size="15" font-weight="900">YIELD</text>');
  if(text.includes('diamond-shaped')) return svg('<rect x="25" y="15" width="70" height="70" transform="rotate(45 60 50)" fill="#f5c928" stroke="#202326" stroke-width="4"/><path d="M56 25h8v21h21v8H64v21h-8V54H35v-8h21z" fill="#202326"/>');
  if(text.includes('five-sided')) return svg('<polygon points="60,6 108,39 91,94 29,94 12,39" fill="#cdff00" stroke="#202326" stroke-width="4"/><g transform="translate(269.7134 2.415271)" fill="#202326"><path fill-rule="evenodd" d="M-200.0883975 29.584754c-.375.29875-.77625.5675-1.21875.75-.45.1825-.92.29125-1.40625.3125-.34625.015-.69.0038-1.03125-.0625l-.15625 1-2.34375 2.15625-2.90625 9.8125-4.25 1.75-2.40625-5.3125-3.6875-2.09375c-.29625.16875-.5775.3425-.90625.4375-.30625.0875-.61625.15375-.9375.15625-.4675.0075-.92375-.03125-1.375-.15625h-.09375l-2.59375 2.75-2.5625 8.21875-4.90625 4.65625 1.40625 1.34375 5.8125-3.84375.75-3.0625.9375 2.84375-3.75 11.5625 1.3125.40625-3.90625 14.75 2.09375.75 5.5625-14.75 2.25.15625 6.15625 14.3125 2.1875-.65625-4.4375-13.90625c.4475-.0975.89375-.215 1.34375-.3125.3925-.09.77375-.22 1.15625-.34375l-2.25-6.90625 5.59375-1.34375-1.15625-5.65625h-1.34375l.90625-1.40625-.15625-.34375 4.8125-1.3125 1.25-2.1875 1.84375 7.6875-3.59375 11.3125-1 17.21875 3.9375.34375 1.40625-14.3125 2.40625-5.34375 1.84375 7.34375 6.8125 12.71875 3.59375-1.75-6.40625-13.21875.65625-11.40625 6.8125-2-1.8125-6.65625-1.75.25.5625-3.59375-5.5625-10.5625-3-1.34375-.5-1.15625zm3.09375 10.0625 3 4.59375-.75 2.8125-1 .25-1.25-7.65625zm-19.5625 6.65625.75 1.59375-.6875 2-.90625.15625.84375-3.75zM-199.2509 25.588479c-.04 1.5425-.6775 2.83875-1.96375 3.72625-1.4925 1.03-3.48875.99625-4.98-.0225-1.2175-.83-1.96625-2.22875-1.9625-3.70375 0-1.7925 1.09625-3.425 2.73875-4.13 1.38125-.58875 2.96875-.4425 4.2125.405 1.21125.8275 1.99 2.25 1.955 3.725"/><path fill-rule="evenodd" d="M-217.4834 33.662229c-.05 1.735-1.04875 3.22625-2.655 3.91375-1.875.8-4.04.0775-5.15125-1.6125-1.085-1.64875-.8325-3.92625.55375-5.31625 1.3875-1.39375 3.66875-1.625 5.31625-.55 1.15125.75625 1.97875 2.165 1.93625 3.565"/></g>');
  if(text.includes('orange-and-black')) return svg('<rect x="25" y="15" width="70" height="70" transform="rotate(45 60 50)" fill="#f28c28" stroke="#202326" stroke-width="4"/><text x="60" y="47" text-anchor="middle" fill="#202326" font-family="Arial,sans-serif" font-size="17" font-weight="900">ROAD</text><text x="60" y="67" text-anchor="middle" fill="#202326" font-family="Arial,sans-serif" font-size="17" font-weight="900">WORK</text>');
  if(text.includes('pink-and-black')) return svg('<rect x="25" y="15" width="70" height="70" transform="rotate(45 60 50)" fill="#ff78a8" stroke="#202326" stroke-width="4"/><text x="60" y="54" text-anchor="middle" fill="#202326" font-family="Arial,sans-serif" font-size="11" font-weight="900">INCIDENT</text>');
  if(text.includes('red circle')) return svg('<circle cx="60" cy="50" r="39" fill="#fff" stroke="#d0202d" stroke-width="8"/><path d="M39 72V51q0-20 20-20h14V22l18 15-18 16v-9H60q-8 0-8 8v20z" fill="#202326"/><path d="M32 78L88 22" stroke="#d0202d" stroke-width="9"/>');
  ...
  if(text.includes('green road')) return svg('<rect x="8" y="18" width="104" height="64" rx="5" fill="#16764a" stroke="#fff" stroke-width="3"/><text x="18" y="42" fill="#fff" font-family="Arial,sans-serif" font-size="10" font-weight="800">VIRGINIA BEACH</text><path d="M26 63h62m0 0-12-9m12 9-12 9" fill="none" stroke="#fff" stroke-width="5"/>');
  ...
  if(text.includes('blue-and-white')) return svg('<rect x="21" y="9" width="78" height="82" rx="5" fill="#1769aa" stroke="#fff" stroke-width="4"/><text x="60" y="63" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="40" font-weight="900">H</text>');
  if(text.includes('brown-and-white')) return svg('<rect x="10" y="17" width="100" height="66" rx="5" fill="#77502d" stroke="#fff" stroke-width="4"/><text x="20" y="43" fill="#fff" font-family="Arial,sans-serif" font-size="11" font-weight="800">STATE PARK</text><path d="M26 63h62m0 0-12-9m12 9-12 9" fill="none" stroke="#fff" stroke-width="5"/>');
  if(text.includes('vertical rectangular')) return svg('<rect x="31" y="6" width="58" height="88" rx="4" fill="#fff" stroke="#202326" stroke-width="4"/><text x="60" y="34" text-anchor="middle" fill="#202326" font-family="Arial,sans-serif" font-size="11" font-weight="900">SPEED</text><text x="60" y="49" text-anchor="middle" fill="#202326" font-family="Arial,sans-serif" font-size="11" font-weight="900">LIMIT</text><text x="60" y="79" text-anchor="middle" fill="#202326" font-family="Arial,sans-serif" font-size="28" font-weight="900">35</text>');
  if(text.includes('do not enter')) return svg('<circle cx="60" cy="50" r="43" fill="#c9232a" stroke="#fff" stroke-width="4"/><text x="60" y="32" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="9" font-weight="900">DO NOT</text><rect x="24" y="42" width="72" height="16" rx="2" fill="#fff"/><text x="60" y="76" text-anchor="middle" fill="#fff" font-family="Arial,sans-serif" font-size="9" font-weight="900">ENTER</text>');
  if(text.includes('winding-road')) return svg('<rect x="25" y="15" width="70" height="70" transform="rotate(45 60 50)" fill="#f5c928" stroke="#202326" stroke-width="4"/><path d="M53 76c25-12 24-24 7-31-11-6-8-10 3-13" fill="none" stroke="#202326" stroke-width="7"/><path d="M63 23l-5 12 10-1z" fill="#202326"/>');
  if(text.includes('crossbuck')) return svg('<g transform="translate(60 50) rotate(38)"><rect x="-57" y="-11" width="114" height="22" fill="#fff" stroke="#202326" stroke-width="3"/></g><g transform="translate(60 50) rotate(-38)"><rect x="-57" y="-11" width="114" height="22" fill="#fff" stroke="#202326" stroke-width="3"/></g><g fill="#202326" font-family="Arial,sans-serif" font-size="8" font-weight="900" text-anchor="middle"><g transform="translate(60 50) rotate(38)"><text x="-29" y="3">RAIL</text><text x="29" y="3">ROAD</text></g><g transform="translate(60 50) rotate(-38)"><text x="-28" y="3">CROSS</text><text x="29" y="3">ING</text></g></g>');
...
function signPng(question) {
  if(question.visual) return `<img src="/assets/signs/${question.visual}.png" alt="" width="120" height="100">`;
  const text=question.q.toLowerCase();
  const assets=[
...
function renderQuestion() {
  ...
  if(q.type==='sign' || q.visual) icon.innerHTML=signPng(q); else icon.textContent=q.icon;
